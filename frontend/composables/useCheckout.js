...
export function useCheckout(shippingRef, selectedShippingMethod, selectedAddress, storeNotes) {
  ...
  try {
    ...
    if (!service) {
      ...
      shopServiceIds.value[sellerId] = service.service_id; // Cập nhật service_id nếu chưa có
    }
    ...
    const { data } = await response.json();
    const shippingFee = data?.total || 0; // Giữ nguyên giá trị từ API
...
const loadShippingFees = async () => {
  ...
  for (const store of cartItems.value) {
    ...
    store.shipping_fee = fee; // Lưu phí vận chuyển vào store
    if (service_id) {
      shopServiceIds.value[store.seller_id] = service_id; // Lưu service_id
    }
  ...
  try {
    ...
    cartItems.value.forEach(store => {
      ...
      if (store.items && Array.isArray(store.items)) {
        const serviceId = shopServiceIds.value[store.seller_id] || 53321;
