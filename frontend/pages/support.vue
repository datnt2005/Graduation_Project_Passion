<template>
  ...
  <div class="max-w-7xl mx-auto">
    <div class="text-sm text-gray-500 px-4 py-2 rounded">
      ...
      <span class="text-black font-medium">Hỗ trợ</span>
    </div>
    <!-- HERO + SEARCH -->
    ...
    <div class="flex flex-col md:flex-row gap-4 mb-4">
      ...
      <!-- FORM LIÊN HỆ -->
      <div class="bg-white border shadow py-6 px-4 rounded w-full md:w-1/2">
        ...
        <form class="space-y-3 text-left" @submit.prevent="submitSupport">
          <input v-model="form.name" type="text" placeholder="Tên của bạn"
            class="w-full px-3 py-2 rounded border text-sm" required />
          <input v-model="form.email" type="email" placeholder="Email" class="w-full px-3 py-2 rounded border text-sm"
            required />
          ...
          <textarea v-model="form.content" rows="4" placeholder="Nội dung"
            class="w-full px-3 py-2 rounded border text-sm" required></textarea>
          <button type="submit"
