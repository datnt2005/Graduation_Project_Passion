<template>
    <div class="bg-gray-100 text-gray-700 font-sans min-h-screen">
        ...
        <div class="px-6 pb-4">
            <NuxtLink to="/admin/reports/reviews/list-reports" class="text-gray-600 hover:underline text-sm">
                Danh sách báo cáo
            </NuxtLink>
...
async function fetchDetail() {
    ...
    try {
        ...
        if (!id) throw new Error('Thiếu ID trong route')
...
async function updateStatus(status) {
    try {
        const token = localStorage.getItem('access_token')
        await axios.put(`${apiBase}/admin/reports/reviews/${route.params.id}/status`, { status }, {
            headers: { Authorization: `Bearer ${token}` }
        })
        await fetchDetail()
    } catch (error) {
