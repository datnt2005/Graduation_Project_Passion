    ...
    <div class="max-w-full overflow-x-auto">
      ...
      <div class="bg-white px-4 py-3 flex items-center justify-between border-t border-gray-300">
        <div class="text-sm text-gray-600">
          Hiển thị {{ paginatedTags.length }} trong số {{ filteredTags.length }} thẻ
        </div>
...
const fetchTags = async () => {
  try {
    ...
    const result = await secureFetch(`${apiBase}/tags`, {
      headers: {
    ...
    totalTags.value = result.data.length;
  } catch (error) {
    console.error('Lỗi khi fetch tags:', error);
    showNotificationMessage('Lỗi khi tải danh sách thẻ', 'error');
  } finally {
    loading.value = false;
  }
};
...
const lastPage = computed(() => {
  return Math.ceil(filteredTags.value.length / perPage.value) || 1;
});
...
const toggleSelectAll = () => {
  if (selectAll.value) {
    selectedTags.value = paginatedTags.value.map(t => t.id);
  } else {
    selectedTags.value = [];
  }
};
