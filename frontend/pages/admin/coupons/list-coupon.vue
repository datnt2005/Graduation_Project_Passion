<template>
  ...
  // Đặt filteredCoupons TRƯỚC các computed khác sử dụng nó
  ...
  const fetchCoupons = async () => {
    try {
      const data = await secureFetch(`${apiBase}/discounts`, {}, ['admin']);
      // Lọc lại: chỉ lấy mã giảm giá không có seller_id (tức là của admin)
      const adminCoupons = data.data.filter(coupon => !coupon.seller_id);
  ...
  const confirmDelete = async (coupon) => {
    showConfirmationDialog(
      ...
      async () => {
        try {
          const result = await secureFetch(`${apiBase}/discounts/${coupon.id}`, {
            method: 'DELETE',
            headers: { 'Content-Type': 'application/json' }
          }, ['admin']);
