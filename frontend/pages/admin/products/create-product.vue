...
const fetchSellers = async () => {
  try {
    ...
    if (!response.success) throw new Error(`HTTP error! status: ${response.status}`);
    const sellerArray = extractArray(response, 'data');
    if (sellerArray.length) {
      sellers.value = sellerArray.map(item => ({
        id: item.id,
        store_name: item.store_name || 'Không có tên'
      }));
    ...
    apiErrors.sellers = 'Không thể tải danh sách người bán. Vui lòng thử lại.';
  }
};
...
const validateFormData = () => {
  ...
  if (!formData.sellers) {
    ...
    errorsList.value.push('Vui lòng chọn người bán.');
    isValid = false;
  } else if (!sellers.value.some(s => s.id === formData.sellers)) {
    errors.sellers = 'Người bán không hợp lệ.';
    errorsList.value.push('Người bán không hợp lệ.');
    isValid = false;
  }
...
const createProduct = async () => {
  ...
  try {
    ...
    }, ['admin'], true); // Cho phép response không success để xử lý lỗi
...
onMounted(async () => {
  ...
  await Promise.allSettled([fetchCategories(), fetchTags(), fetchAttributes(), fetchSellers()]).then(([catResult, tagResult, attrResult, sellerResult]) => {
    console.log('Fetch results:', { catResult, tagResult, attrResult, sellerResult });
