<template>
  <div class="bg-[#f5f5f5] font-sans text-[14px] text-[#222222]">
    <div class="max-w-7xl mx-auto px-4 py-4 flex flex-col md:flex-row md:space-x-4">
      <!-- Left main content -->
      <div class="flex-1">
        <!-- Breadcrumb -->
        ...
        <template v-else>
          <!-- Desktop header -->
          <div
            class="hidden md:grid grid-cols-[40px_280px_1fr_120px_120px_120px_40px] items-center bg-white text-[#999999] text-[13px] font-normal rounded-md px-4 py-2 mb-2 select-none">
          ...
          <div v-for="store in cart.stores" :key="store.seller_id" class="mb-4 border border-[#f8b77b] rounded-md">
            ...
            <div class="flex items-center space-x-2 px-4 py-2 border-b border-[#f8b77b] bg-[#ffe0b3]">
              <label class="flex items-center space-x-2 text-[13px] font-normal flex-1">
                <input type="checkbox" :checked="storeSelections[store.seller_id]"
                  @change="handleStoreSelection(store, $event)"
                  class="w-4 h-4 border border-[#d9d9d9] rounded-sm checked:bg-[#007aff] cursor-pointer" />
                <i class="fas fa-store-alt text-[#666]"></i>
                <NuxtLink :to="store.store_url" class="text-[#222] hover:underline">{{ store.store_name }}</NuxtLink>
                <i class="fas fa-chevron-right text-xs text-[#666]"></i>
              </label>
              <button class="text-[#007aff] text-[13px] hover:underline" @click="selectStoreItems(store)">
                Chọn tất cả sản phẩm
              </button>
            </div>

            <!-- Product items -->
            <div v-for="item in store.items" :key="item.id" class="border-b last:border-b-0">
              <!-- Mobile card -->
              <div class="md:hidden flex items-start space-x-3 p-3 bg-white">
                <input type="checkbox" :checked="selectedItems.has(item.id)"
                  @change="handleItemSelection(item.id, $event)"
                  class="mt-3 w-4 h-4 border border-[#d9d9d9] rounded-sm checked:bg-[#F97316] cursor-pointer" />
                <img :src="`${mediaBaseUrl}${item.productVariant.thumbnail}`" :alt="item.product.name"
                  class="w-[70px] h-[70px] object-cover rounded-md border" />
                <div class="flex-1">
                  <div class="text-[14px] font-semibold leading-snug line-clamp-2">
                    {{ item.product.name }}
                  </div>
                  ...
                  <div class="flex items-center justify-between mt-2">
                    ...
                    <div class="flex items-center border rounded">
                      ...
                      <button @click="
                        ...
                        " class="w-7 h-7 text-lg font-semibold border-l"
              ...
              <!-- Desktop row -->
              <div
                class="hidden md:grid md:grid-cols-[40px_280px_1fr_120px_120px_120px_40px] gap-2 px-4 py-3 items-center">
                <!-- Checkbox -->
                ...
                <div class="flex items-center space-x-3">
                  ...
                  <div class="flex flex-col text-[13px] leading-tight">
                    <NuxtLink :to="`/products/${item.product.slug}`" class="text-[#222] font-semibold hover:underline">
                      {{ item.product.name }}
                    </NuxtLink>
                ...
                <!-- Đơn giá -->
                <div class="text-right text-[14px] font-semibold text-red-500">
                  {{ formatPrice(item.sale_price || item.price) }}
                </div>
                ...
                <div class="flex justify-center items-center space-x-1">
                  <button @click="
                    updateQuantityWithValidation(item.id, item.quantity - 1)
                    " class="border border-[#d9d9d9] rounded-l-sm w-6 h-6 text-sm font-semibold"
                    :disabled="item.quantity <= 1 || loading">
                    −
                  </button>
                  <input type="number" v-model.number="item.quantity" @change="
                    updateQuantityWithValidation(item.id, item.quantity)
                    " :disabled="loading" class="w-10 h-6 border-t border-b text-center text-sm font-semibold" />
                  <button @click="
                    updateQuantityWithValidation(item.id, item.quantity + 1)
                    " class="border border-[#d9d9d9] rounded-r-sm w-6 h-6 text-sm font-semibold"
                    :disabled="item.quantity >= item.stock || loading">
                    +
                  </button>
                </div>
                ...
                <div class="text-right text-[#ff3b30] font-semibold text-[14px]">
                  {{
                    formatPrice(
                      parsePrice(item.sale_price || item.price) * item.quantity
                    )
                  }}
                </div>
