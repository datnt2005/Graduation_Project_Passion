    ...
    <div class="max-w-full overflow-x-auto">
      ...
      <div class="bg-gray-200 px-4 py-3 flex flex-wrap items-center gap-3 text-sm">
        ...
        <div class="ml-auto relative">
          <input
            ...
            class="pl-8 pr-3 py-1.5 rounded-md border border-gray-300 focus:ring-blue-500 w-64"
          />
      ...
      <table class="min-w-full border-collapse border border-gray-300 text-sm">
        <thead class="bg-white">
          <tr>
            <th v-for="header in headers" :key="header" class="border px-3 py-2 text-left font-semibold text-gray-700">
              {{ header }}
            </th>
          </tr>
        </thead>
        <tbody v-if="!isLoading">
          <tr v-for="item in paginatedInventories" :key="item.id" :class="{ 'bg-gray-50': item.id % 2 === 0 }">
            ...
            <td
              ...
              :class="{
                'bg-red-100 text-red-700 font-semibold': item.quantity === 0,
                'bg-yellow-50 text-yellow-800 font-medium': item.quantity > 0 && item.quantity < 10,
                'text-gray-700': item.quantity >= 10,
              }"
            >
            ...
            <td class="border px-3 py-2 relative" data-dropdown>
              <div class="relative inline-block text-left">
                ...
                <div
                  v-if="item.showMenu"
                  class="absolute right-0 z-10 mt-1 w-32 bg-white border border-gray-200 rounded shadow-md"
                >
                  <button
        ...
        <tbody v-else>
          <tr v-for="n in 5" :key="n" class="animate-pulse bg-white">
            <td v-for="i in headers.length" :key="i" class="border px-3 py-2">
      ...
      <div class="bg-white px-4 py-3 flex items-center justify-between border-t border-gray-200">
        <div class="text-sm">
          Hiển thị {{ (currentPage - 1) * itemsPerPage + 1 }} - {{ Math.min(currentPage * itemsPerPage, filteredInventories.length) }} trong {{ filteredInventories.length }} sản phẩm
        </div>
      ...
      <InventoryModal
        ...
        @close="showModal = false; selectedInventory = null"
        @submitted="onInventorySubmitted"
      />

      <!-- Lịch sử biến động kho -->
      <StockMovementHistory :productVariantId="selectedInventory?.product_variant_id || null" :refreshKey="stockRefreshKey" />
    </div>
  </div>
</template>
...
const stockRefreshKey = ref(0); // For StockMovementHistory refresh
...
const fetchInventories = async () => {
  ...
  try {
    const { data } = await secureAxios(`${apiBase}/inventory/list?t=${Date.now()}`, {}, ['admin', 'seller']);
...
const onInventorySubmitted = async () => {
