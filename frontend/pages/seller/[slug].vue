  ...
  <main class="bg-[#F5F5FA] py-2">
    <div class="w-full max-w-[1300px] mx-auto mb-4">
      <div class="text-sm text-gray-500 px-4 py-2 rounded">
        ...
        <span class="text-black font-medium">Cửa hàng</span>
      </div>
    </div>
    <div
      class="max-w-[1300px] bg-white p-4 min-h-screen shadow w-full mx-auto mt-4"
      v-if="seller"
    >
      <div v-if="loading" class="animate-pulse">
          ...
          <div class="flex flex-col gap-4">
            ...
            <div class="flex flex-wrap gap-2">
              <div
                v-for="n in 3"
                :key="n"
                class="h-12 w-32 bg-gray-200 rounded-lg"
              ></div>
            </div>
          </div>
        ...
        <div
          class="mt-6 border-t pt-6 flex flex-col lg:flex-row justify-between gap-4"
        >
          <div class="flex flex-wrap gap-3">
            <div
              v-for="n in 4"
              :key="n"
              class="h-8 w-24 bg-gray-200 rounded-md"
            ></div>
          </div>
          ...
          <aside class="bg-white p-5 border-r min-h-screen col-span-1">
            ...
            <ul class="space-y-2">
              <li
                v-for="n in 5"
                :key="n"
                class="h-8 w-full bg-gray-200 rounded"
              ></li>
            </ul>
          </aside>
          <section class="col-span-1 md:col-span-4">
            <div
              class="bg-white p-3 shadow rounded mb-4 flex flex-wrap justify-between items-center"
            >
      ...
      <div v-else>
        ...
        >
          <div class="flex flex-col gap-4">
            <div class="flex items-center gap-4">
              <div
                class="w-14 h-14 bg-gray-200 rounded-full flex items-center justify-center text-2xl"
              >
                <img
                  v-if="seller.avatar"
                  :src="
            ...
            <div v-if="quickVouchers.length > 0" class="flex flex-wrap gap-3">
              <div
                v-for="voucher in quickVouchers"
                :key="voucher.id"
              ...
              >
                ...
                <button
                  ...
                  class="text-xs font-semibold px-2 py-1 rounded-full transition-all duration-200 border"
                  :class="{
                    'bg-white text-blue-600 border-blue-500 hover:bg-blue-50':
                      voucher.is_saved && !isVoucherUnavailable(voucher),
                    'bg-blue-600 text-white border-blue-600 hover:bg-blue-700':
                      !voucher.is_saved && !isVoucherUnavailable(voucher),
                    'bg-gray-400 text-white border-gray-400 cursor-not-allowed':
                      isVoucherUnavailable(voucher),
                  }"
                >
            ...
            <div
              ...
              class="flex flex-wrap gap-2 animate-pulse"
            >
              <div
                v-for="n in 3"
                :key="n"
        ...
        <div
          class="mt-6 border-t pt-6 flex flex-col lg:flex-row justify-between gap-4 items-start lg:items-center"
        >
          <nav class="flex flex-wrap gap-3 text-sm font-medium text-gray-700">
            <a
              ...
              :class="{
                'bg-blue-100 text-blue-600': activeTab === tab,
                'hover:text-blue-600 hover:bg-blue-50': activeTab !== tab,
              }"
          ...
          <div class="w-full lg:w-1/4" v-if="activeTab === 'Cửa hàng'">
            <div class="flex border rounded overflow-hidden max-w-full">
              ...
              />
              <button
                class="bg-gray-100 px-4 text-sm hover:bg-gray-200 transition"
              >
                Tìm
              </button>
            </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-5 gap-4 mt-6">
          <!-- PHẦN CHO DESKTOP -->
          <aside
            class="hidden md:block bg-white p-5 border-r min-h-screen col-span-1"
            v-if="activeTab === 'Cửa hàng'"
          >
            <h3
              class="font-semibold text-base mb-4 text-gray-800 border-b pb-2"
            >
              Tất cả danh mục
            </h3>
          ...
          <!-- HEADER + MENU NỔI (MOBILE) -->
          <div
            class="relative md:hidden border-b"
            v-if="activeTab === 'Cửa hàng'"
          >
            <!-- Tiêu đề + nút -->
            ...
            <transition name="fade">
              <div
                v-if="showMobileMenu"
                class="absolute left-0 top-full z-50 w-full bg-white border-t border-b shadow-md"
              >
                <ul class="p-1 space-y-2 text-gray-700 text-sm">
                  <li v-for="category in uniqueCategories" :key="category">
                    <a
                      href="#"
                      @click.prevent="filterByCategory(category)"
          ...
          >
            ...
            <div v-if="activeTab === 'Cửa hàng'">
              <div
                class="bg-white p-3 shadow rounded mb-4 flex flex-wrap justify-between items-center text-sm"
              >
                <h3 class="font-semibold mb-2 md:mb-0">
                  Tất cả sản phẩm: {{ filteredProducts.length }}
                </h3>

                <!-- Thanh cuộn ngang trên mobile -->
                <div
                  class="flex overflow-x-auto whitespace-nowrap gap-3 font-medium text-sm pb-2"
                >
                  <button
                    v-for="(label, sortKey) in {
                      popular: 'Phổ biến',
                      sold: 'Bán chạy',
                      new: 'Hàng mới',
                    ...
                    :key="sortKey"
                    @click="sortProducts(sortKey)"
                    class="px-3 py-1 rounded transition shrink-0"
                    :class="
                      activeSort === sortKey
                        ? 'bg-blue-100 text-blue-600 font-semibold'
              ...
              <div
                ...
                class="text-center py-8 text-gray-400 text-base"
              >
              ...
              <!-- Pagination -->
              <div
                class="mt-8 flex justify-center items-center gap-1 text-sm flex-wrap"
              ...
              >
                ...
                <template v-for="(page, i) in visiblePages" :key="i">
                  <span
                    v-if="page === '...'"
                    class="px-3 py-1 text-gray-400 font-semibold select-none"
                    >...</span
                  >
                  <button
                    v-else
                    class="px-3 py-1 rounded-full border transition font-semibold shadow-sm"
                    :class="
            ...
            <div v-if="activeTab === 'Giá sốc hôm nay'">
              <div class="bg-white p-3 shadow rounded mb-4">
                ...
                <div
                  v-else-if="errorDeals"
                  class="text-center py-4 text-red-500"
                >
                  Lỗi: {{ errorDeals }}
                </div>
                <div
                  v-else-if="dealProducts.length === 0"
                  class="text-center py-8 text-gray-400 text-base"
                >
            ...
            <div v-if="activeTab === 'Hồ sơ cửa hàng'" class="py-6">
              ...
              >
                <h3
                  class="font-bold text-xl mb-6 text-gray-900 border-b-2 border-blue-100 pb-3"
                >
                  Hồ sơ cửa hàng
                </h3>
                <div
                  v-if="isLoggedIn && !isNotOwner"
                  class="mb-4 bg-green-50 p-4 rounded-lg"
                >
                  <p class="text-sm text-green-700">
                    Quản lý cửa hàng của bạn: chỉnh sửa thông tin, xem đơn hàng,
                    hoặc theo dõi người theo dõi.
                  </p>
                ...
                <div class="grid grid-cols-1 lg:grid-cols-2 gap-8 text-sm">
                  <div class="space-y-6">
                    ...
                    >
                      <div class="flex items-center gap-3">
                        <span class="text-blue-700 font-semibold"
                          >Thành viên từ năm</span
                        >
                      </div>
                      <span class="text-gray-900 font-medium">{{
                        seller.member_since || "N/A"
                      }}</span>
                    </div>
                    <div
                      class="flex items-center justify-between p-4 bg-blue-50 rounded-lg hover:bg-blue-100 transition-colors duration-200"
                  ...
                  <div class="space-y-6">
                    ...
                    >
                      ...
                      <p
                        class="text-gray-700 leading-relaxed text-base min-h-[40px]"
                      >
                    ...
                    >
                      ...
                      <div class="flex items-center justify-between">
                        ...
                        <span
                          class="flex items-center gap-1 text-yellow-500 font-bold text-xl"
                        >
                          {{
                            seller.stars
                              ? "★".repeat(seller.stars) +
                                "☆".repeat(5 - seller.stars)
                              : "☆☆☆☆☆"
                          }}
                          <span class="text-gray-700 ml-2 text-base"
                            >({{ seller.rating || "0" }}/5)</span
                          >
                        </span>
                      </div>
                    </div>
            ...
            <div v-if="activeTab === 'Voucher của shop'" class="py-6">
              <div
                class="bg-white p-6 shadow-lg rounded-xl border border-gray-100"
              >
                <h3
                  class="font-bold text-xl mb-6 text-gray-900 border-b-2 border-blue-100 pb-3"
                >
                  ...
                  >
                    ...
                    >
                      <img
                        v-if="voucher.seller"
                        :src="
                          voucher.seller.avatar.startsWith('http')
                            ? voucher.seller.avatar
                            : `${mediaBase}${voucher.seller.avatar}`
                        "
                        alt="Brand"
                        class="w-full h-full object-cover"
                      />
                      <span v-else class="text-2xl text-gray-600">{{
                        getBrandInitial(voucher)
                      }}</span>
                    </div>
                    <!-- Voucher Details -->
                    <div class="flex-1">
                      ...
                      <div
                        v-if="voucher.products && voucher.products.length"
                    ...
                    <div class="ml-3">
                      <button
                        @click="handleVoucherAction(voucher)"
                        :disabled="isVoucherUnavailable(voucher)"
                        class="text-sm font-semibold px-4 py-2 rounded-full transition-all duration-200 border"
                        :class="{
                          'bg-white text-blue-600 border-blue-500 hover:bg-blue-50':
                            voucher.is_saved && !isVoucherUnavailable(voucher),
                          'bg-blue-600 text-white border-blue-600 hover:bg-blue-700':
                ...
                >
                  ...
                  <template v-for="(page, i) in visibleVoucherPages" :key="i">
                    ...
                    <button
                      ...
                      class="px-3 py-1 rounded-full border transition font-semibold shadow-sm"
                      :class="
                        page === voucherPagination.current_page
                          ? 'bg-[#1BA0E2] text-white border-[#1BA0E2] shadow-md sm:scale-100 md:scale-105'
                          : 'bg-white border-gray-300 hover:bg-blue-50 hover:border-blue-400 text-gray-700'
                      "
                  ...
                  <button
                    class="px-3 py-1 rounded-full border border-gray-300 bg-white shadow-sm hover:bg-blue-50 hover:border-blue-400 transition disabled:opacity-50 disabled:cursor-not-allowed"
                    :disabled="
                      voucherPagination.current_page ===
                      voucherPagination.last_page
                    "
                    @click="fetchVouchers(voucherPagination.current_page + 1)"
                  >
                    <i class="fas fa-chevron-right ml-1"></i>
...
const isNotOwner = computed(() => {
  ...
  return (
    !seller.value.is_owner && currentUser.value.id !== seller.value.user_id
  );
});
...
const viewFollowers = () => {
  router.push(`/sellers/${seller.value.id}/followers`);
};
...
const fetchSeller = async (page = 1) => {
  try {
    ...
    const res = await axios.get(
      `${apiBase}/sellers/store/${route.params.slug}`,
      {
        params: {
          page,
          per_page: 24,
    ...
    seller.value = {
      ...sellerData,
      is_owner:
        sellerData.is_owner ?? currentUser.value?.id === sellerData.user_id,
    };
    ...
    if (!success.success)
      error.value =
        err.response?.data?.message || "Không thể tải dữ liệu cửa hàng.";
  } finally {
    loading.value = false;
  }
};
...
const fetchDeals = async () => {
  try {
    ...
    const res = await axios.get(
      `${apiBase}/sellers/store/${route.params.slug}/deals`,
      {
        params: { per_page: 24 },
        headers: {
          Authorization: `Bearer ${localStorage.getItem("access_token")}`,
    ...
    if (!success.success)
      errorDeals.value =
        err.response?.data?.message || "Không thể tải ưu đãi hôm nay.";
  } finally {
...
const fetchVouchers = async (page = 1, limit = 9) => {
  ...
  try {
    ...
    const res = await axios.get(
      `${apiBase}/sellers/store/${route.params.slug}/discounts`,
      {
    ...
    const currentVouchers = new Map(
      vouchers.value.map((v) => [v.code, v.is_saved])
    );
    const newVouchers = voucherData.map((voucher) => {
      const isSaved = currentVouchers.has(voucher.code)
        ? currentVouchers.get(voucher.code)
        : voucher.is_saved || false;
      return {
        ...voucher,
        ...
        usage_limit:
          voucher.usage_limit !== null ? parseInt(voucher.usage_limit) : null,
...
const toggleFollow = async () => {
  ...
  try {
    const url = `${apiBase}/sellers/${seller.value.id}/${
    ...
    if (!success.success)
      toast(
        "error",
        err.response?.data?.message || "Lỗi khi thao tác theo dõi."
      );
  } finally {
...
const sortProducts = (sortType) => {
  ...
  const getRatingValue = (rating) =>
    !rating ? 0 : (rating.match(/★/g) || []).length;
  switch (sortType) {
    case "popular":
      sorted.sort(
        (a, b) => getRatingValue(b.rating) - getRatingValue(a.rating)
      );
      break;
    case "sold":
  ...
  if (originalOrder.join() === sorted.map((p) => p.id).join())
    toast("info", "Không có sự thay đổi trong sắp xếp.");
};
...
const setActiveTab = (tab) => {
  ...
  else if (tab === "Voucher của shop" && vouchers.value.length === 0)
    fetchVouchers();
  if (tab !== "Cửa hàng") {
    searchQuery.value = "";
...
onMounted(async () => {
  try {
    ...
    if (route.query.tab) {
      activeTab.value = tabs.includes(route.query.tab)
        ? route.query.tab
        : "Cửa hàng";
      if (activeTab.value === "Giá sốc hôm nay") await fetchDeals();
      else if (
        activeTab.value === "Voucher của shop" &&
