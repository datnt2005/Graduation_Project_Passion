<template>
  <div>
    ...
    <header class="bg-[#1BA0E2] text-white text-sm py-2">
      <!-- DESKTOP & MOBILE -->
      <div class="flex flex-wrap sm:flex-nowrap justify-between items-center px-6 gap-2">
        <!-- BÊN TRÁI - Desktop only -->
        <div class="hidden sm:flex items-center flex-wrap gap-x-6 text-white text-sm">
          <div class="flex items-center gap-1">
            <i class="fa-solid fa-phone"></i>
            <span>Hỗ trợ: <strong>{{ contactHotline }}</strong></span>
          </div>
          <div class="flex items-center gap-1">
        ...
        <div class="flex items-center flex-wrap gap-4 ml-auto">
          ...
          <div class="relative group hidden sm:block">
            <div class="cursor-pointer hover:text-white transition-colors duration-200 flex items-center"
              @click="toggleNotificationDropdown">
              <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-7" fill="none" viewBox="0 0 24 24"
                stroke="currentColor">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5"
                  d="M14.25 17.25a2.25 2.25 0 0 1-4.5 0m8.25-5.25v-1.5a6 6 0 1 0-12 0v1.5c0 .621-.252 1.216-.7 1.65L3.63 15.255A.75.75 0 0 0 4.14 16.5h15.72a.75.75 0 0 0 .51-1.245l-1.42-1.605a2.25 2.25 0 01-.7-1.65Z" />
              </svg>
      ...
      <div class="sm:hidden relative px-4 text-white text-sm mt-2 h-8 flex items-center justify-center">
          ...
          <div v-if="showNotificationModal"
            ...
            <div class="bg-white rounded-lg shadow-lg max-w-md w-full p-6 relative">
              ...
              <div v-if="currentNotification?.image_url" class="mb-4">
                <img :src="currentNotification.image_url" alt="Ảnh"
    ...
    <div class="bg-white shadow-sm">
      <div class="max-w-7xl mx-auto px-2 flex items-center gap-6 justify-between px-2 py-3">
        ...
        <div class="flex items-center gap-3 w-[250px]">
          ...
          <div class="relative group hidden sm:block mx-5">
            ...
            <div
              ...
              <div>
                <h4 class="font-bold mb-2 text-gray-800 text-sm sm:text-base">Sản phẩm nổi bật</h4>
                <ul class="text-gray-700 space-y-1">
                  <li>
                    <NuxtLink to="/shop/new" class="hover:underline text-sm">Sản phẩm mới</NuxtLink>
                  </li>
                  <li>
                    <NuxtLink to="/shop/sale" class="hover:underline text-sm">Khuyến mãi</NuxtLink>
                  </li>
                </ul>
              </div>
              <div>
              ...
              <div>
                <h4 class="font-bold mb-2 text-gray-800 text-sm sm:text-base">Hợp tác</h4>
                <ul class="text-gray-700 space-y-2">
                  <li>
                    <NuxtLink to="/sell-together-passion" class="flex items-center gap-2 hover:underline">
                      <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 text-[#1BA0E2]" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.8">
        ...
        <!-- Biểu tượng bên phải -->
        <div class="hidden sm:flex items-center gap-x-6 text-sm font-medium text-gray-700 flex-shrink-0">
          <NuxtLink to="/"
          ...
          <div class="relative group inline-block">
            ...
            <ul v-if="isLoggedIn"
              ...
              <li>
                <NuxtLink to="/users/profile" class="block px-4 py-2 hover:bg-gray-100">Thông tin tài khoản</NuxtLink>
              </li>
              <li>
                <NuxtLink to="/users/orders" class="block px-4 py-2 hover:bg-gray-100">Đơn hàng của tôi</NuxtLink>
              </li>
              <li>
                <NuxtLink to="/support" class="block px-4 py-2 hover:bg-gray-100">Trung tâm hỗ trợ</NuxtLink>
              </li>
              <li v-if="userRole === 'seller'">
                <NuxtLink to="/seller/dashboard" class="block px-4 py-2 hover:bg-gray-100">Quản lý cửa hàng</NuxtLink>
              </li>
              <li v-if="userRole === 'admin'">
                <NuxtLink to="/admin/dashboard" class="block px-4 py-2 hover:bg-gray-100">Quản lý hệ thống</NuxtLink>
              </li>
            </ul>
          </div>
    ...
    <div v-if="isMobileMenuOpen"
      ...
      <div class="w-[80%] max-w-[400px] h-full bg-white shadow-md p-6 relative animate-slide-in-right">
        ...
        <div class="mt-10 space-y-4 text-sm font-medium text-gray-700">
          <!-- Trang chủ -->
          <NuxtLink to="/" class="flex items-center gap-2 hover:text-blue-600 transition-colors duration-200"
          ...
          <div class="relative">
            <button @click="isCategoryMenuOpen = !isCategoryMenuOpen"
              ...
              Danh mục
              <font-awesome-icon :icon="['fas', isCategoryMenuOpen ? 'chevron-up' : 'chevron-down']" class="ml-auto" />
            </button>
            <div v-if="isCategoryMenuOpen" class="mt-2 pl-6 space-y-2">
              <div>
                <h4 class="font-bold text-gray-800">Sản phẩm nổi bật</h4>
                <ul class="text-gray-700 space-y-1 mt-1">
                  <li>
                    <NuxtLink to="/shop/new" class="hover:underline" @click="isMobileMenuOpen = false">Sản phẩm mới
                    </NuxtLink>
                  </li>
                  <li>
                    <NuxtLink to="/shop/sale" class="hover:underline" @click="isMobileMenuOpen = false">Khuyến mãi
                    </NuxtLink>
                  </li>
                </ul>
              </div>
              <div>
                <h4 class="font-bold text-gray-800">Danh mục sản phẩm</h4>
                <ul class="text-gray-700 space-y-2 mt-1">
                  <li v-for="(item, index) in categories" :key="index">
                    <NuxtLink :to="`/shop/${item.slug}`" class="flex items-center gap-2 hover:underline"
                      @click="isMobileMenuOpen = false">
                      <img :src="`${mediaBase}${item.image}`" :alt="item.name"
              ...
              <div>
                <h4 class="font-bold text-gray-800">Hợp tác</h4>
                <ul class="text-gray-700 space-y-2 mt-1">
                  <li>
                    <NuxtLink to="/sell-together-passion" class="flex items-center gap-2 hover:underline"
                      @click="isMobileMenuOpen = false">
          ...
          <div class="relative">
            <button @click="isAccountMenuOpen = !isAccountMenuOpen"
              ...
              <span>Tài khoản</span>
              <font-awesome-icon :icon="['fas', isAccountMenuOpen ? 'chevron-up' : 'chevron-down']" class="ml-auto" />
            </button>
            <ul v-if="isLoggedIn && isAccountMenuOpen" class="mt-2 pl-6 space-y-2 text-gray-700">
              <li>
                <NuxtLink to="/users/profile" class="block hover:underline" @click="isMobileMenuOpen = false">Thông tin
                  tài
                  khoản</NuxtLink>
              </li>
              <li>
                <NuxtLink to="/users/orders" class="block hover:underline" @click="isMobileMenuOpen = false">Đơn hàng
                  của
                  tôi</NuxtLink>
              </li>
              <li>
                <NuxtLink to="/support" class="block hover:underline" @click="isMobileMenuOpen = false">Trung tâm hỗ trợ
                </NuxtLink>
              </li>
              <li v-if="userRole === 'seller'">
                <NuxtLink to="/seller/dashboard" class="block hover:underline" @click="isMobileMenuOpen = false">Quản lý
                  cửa
                  hàng</NuxtLink>
              </li>
              <li v-if="userRole === 'admin'">
                <NuxtLink to="/admin/dashboard" class="block hover:underline" @click="isMobileMenuOpen = false">Quản lý
                  hệ
                  thống</NuxtLink>
              </li>
            </ul>
          ...
          <!-- Giỏ hàng -->
          <NuxtLink to="/cart"
            class="flex items-center gap-2 hover:text-blue-600 transition-colors duration-200 relative"
            @click="isMobileMenuOpen = false">
            <div class="relative">
              <font-awesome-icon :icon="['fas', 'cart-shopping']" class="w-5 h-5" />
              <span v-if="cartStore.totalItems > 0"
                class="absolute -top-2 -right-2 bg-red-500 text-white text-xs font-semibold rounded-full w-4 h-4 flex items-center justify-center">
                {{ cartStore.totalItems }}
              </span>
          ...
          <div class="relative">
            <button @click="isNotificationMenuOpen = !isNotificationMenuOpen"
              ...
              <span>Thông báo</span>
              <span v-if="unreadCount > 0"
                class="ml-2 bg-red-500 text-white text-xs rounded-full px-1.5 min-w-[20px] text-center">
                {{ unreadCount }}
              </span>
              <font-awesome-icon :icon="['fas', isNotificationMenuOpen ? 'chevron-up' : 'chevron-down']"
                class="ml-auto" />
            </button>
            <div v-if="isNotificationMenuOpen" class="mt-2 pl-6 space-y-2">
              <div v-if="notifications.length === 0" class="text-gray-500">Không có thông báo mới.</div>
...
const contactHotline = computed(() => {
  return settings.value?.contact?.find((s) => s.key === "hotline_number")?.value || "Chưa cấu hình";
});
