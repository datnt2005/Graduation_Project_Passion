<template>
  <div class="relative w-full" ref="wrapper">
    ...
    <form
      ...
      class="flex items-center w-full max-w-full sm:max-w-[500px] mx-auto border border-gray-200 rounded-full overflow-hidden bg-white shadow-md transition-all duration-300 px-3"
    >
      <!-- Icon tìm kiếm -->
      ...
      <input
        ...
        class="flex-grow px-2 py-2 text-sm focus:outline-none placeholder-gray-400"
      ...
      <button
        type="submit"
        class="text-sm text-blue-600 hover:text-blue-700 transition whitespace-nowrap py-2 px-3"
      >
        Tìm
      </button>
    </form>
    ...
    <div
      ...
      class="absolute left-1/2 transform -translate-x-1/2 z-20 bg-white border border-gray-200 rounded-2xl shadow-lg w-full max-w-full sm:max-w-[500px] mt-2 overflow-hidden transition-all duration-200"
    >
      <!-- Từ khóa nổi bật -->
      <div
        v-if="suggestions.top_keywords.length && !loadingSuggestions"
      ...
      <!-- Gợi ý sản phẩm -->
      <div
        v-if="input.trim() && suggestions.top_products.length"
        class="px-4 py-3 border-b border-gray-100"
      >
        <h3 class="text-sm font-semibold text-gray-800 mb-2">
          Gợi ý cho "{{ input }}"
        ...
        <ul class="space-y-2">
          <li
            ...
            :key="product.id"
            class="flex items-center py-2 px-2 text-sm text-gray-700 hover:bg-gray-100 cursor-pointer rounded-lg transition"
            @click="selectProduct(product.id)"
          >
            <img
              v-if="product.image"
              :src="product.image"
              class="w-8 h-8 mr-3 rounded-full object-cover border"
      ...
      <!-- Lịch sử tìm kiếm -->
      <div
        v-if="suggestions.history.length && !loadingSuggestions"
        class="px-4 py-3 border-b border-gray-100"
      >
        ...
        <ul class="space-y-2">
          <li
            ...
            :key="keyword"
            class="flex justify-between items-center py-2 px-2 text-sm text-gray-700 hover:bg-gray-100 cursor-pointer rounded-lg transition"
            @click="selectSuggestion(keyword)"
          >
      ...
      <div class="px-4 py-3">
        <ul class="space-y-2">
          <li
            v-for="keyword in staticSamples"
            :key="'sample-' + keyword"
            class="flex items-center py-2 px-2 text-sm text-gray-700 hover:bg-gray-100 cursor-pointer rounded-lg transition"
            @click="selectSuggestion(keyword)"
          >
...
// Từ khoá mẫu cố định luôn hiển thị cuối dropdown
const staticSamples = [
  "Điện thoại",
  "Laptop",
  "Áo thun nam",
...
async function removeKeyword(keyword) {
  ...
  try {
    ...
    suggestions.value.history = suggestions.value.history.filter(
      (k) => k !== keyword
    );
  } catch (error) {
