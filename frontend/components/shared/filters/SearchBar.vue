    ...
    <div class="flex-1 mx-4 hidden sm:flex justify-center w-[600px] relative">
      <form @submit.prevent="handleSearch"
        ...
        <input type="text" v-model="input" placeholder="Tìm kiếm sản phẩm..."
          class="flex-grow px-3 py-2.5 text-sm focus:outline-none placeholder-gray-400" @focus="handleInputFocus"
          @input="updateSearch" @keyup.enter="handleSearch" />
        <button type="submit" class="px-4 py-2 text-sm text-blue-600 hover:text-blue-700 transition whitespace-nowrap">
          Tìm kiếm
        </button>
      </form>
      ...
      <div v-if="showSuggestions"
        ...
        <div v-if="suggestions.top_keywords.length && !loadingSuggestions" class="px-5 py-4 border-b border-gray-100">
          ...
          <div class="flex flex-wrap gap-2">
            <span v-for="keyword in suggestions.top_keywords" :key="keyword" @click="selectSuggestion(keyword)"
        ...
        <div v-if="input.trim() && suggestions.top_products.length" class="px-5 py-4 border-b border-gray-100">
          ...
          <ul class="space-y-2">
            <li v-for="product in suggestions.top_products" :key="product.id"
              ...
              <img v-if="product.image" :src="product.image" class="w-9 h-9 mr-3 rounded-full object-cover border"
        ...
        <!-- Lịch sử tìm kiếm -->
        <div v-if="suggestions.history.length && !loadingSuggestions" class="px-5 py-4 border-b border-gray-100">
          <div class="flex justify-between items-center mb-2">
            <h3 class="text-sm font-semibold text-gray-800">Lịch sử tìm kiếm</h3>
            <button class="text-xs text-red-500 hover:text-red-600 font-medium" @click="clearHistory">Xóa tất
              cả</button>
          </div>
          <ul class="space-y-2">
        ...
        <!-- Từ khoá mẫu cố định -->
        <div class="px-5 py-4">
          <!-- <h3 class="text-sm font-semibold text-gray-800 mb-2">Từ khoá gợi ý</h3> -->
          <ul class="space-y-2">
            <li v-for="keyword in staticSamples" :key="'sample-' + keyword"
              class="flex items-center py-2 px-4 text-sm text-gray-700 hover:bg-gray-100 cursor-pointer rounded-lg transition"
              @click="selectSuggestion(keyword)">
