<template>
  ...
  <button
    ...
    @click="chatStore.openChat(); hasNewMessage = false"
  >
    ...
    >
      ...
      <div class="flex items-center justify-between px-4 py-2 bg-[#1BA0E2] text-white">
        <h2 class="font-semibold text-base">
          {{ chatStore.currentSession ? `Đang chat với: ${chatStore.currentSession?.seller?.store_name || 'Cửa hàng'}` : 'Chat với cửa hàng' }}
        </h2>
        <button @click="chatStore.closeChat()" class="hover:opacity-80">
      ...
      <div class="flex-1 flex overflow-hidden" @click="closeContext">
          ...
          <ul class="space-y-2 overflow-y-auto max-h-[400px] pr-1">
            <li
              ...
              :class="[
                ...
                chatStore.currentSession?.id === session.id ? 'bg-blue-100' : 'hover:bg-gray-100'
              ]"
            >
              <img
        ...
        <section class="flex-1 flex flex-col bg-gray-100 overflow-hidden">
          ...
          <div class="flex-1 p-3 space-y-3 overflow-y-auto" ref="chatBox">
            ...
            >
              <!-- Nếu là tin nhắn sản phẩm -->
              <div
                v-if="msg.message_type === 'product'"
                class="relative p-2 bg-white border rounded shadow-sm text-sm w-[260px] sm:max-w-[70%]"
              >
                <img
                ...
                />
                <p class="font-semibold text-sm text-gray-800">{{ msg.message }}</p>
                <a
                  v-if="msg.metadata?.productLink"
                  :href="msg.metadata.productLink"
                  target="_blank"
              ...
              >
                ...
                <!-- File đính kèm -->
                <div
                  v-if="msg.message_type !== 'revoked' && msg.attachments?.length"
              ...
              <!-- Nút 3 chấm -->
              <button
                v-if="msg.sender === 'user' && msg.message_type !== 'revoked'"
              ...
              <div
                v-if="contextMenu.open && contextMenu.messageId === msg.id && msg.sender === 'user'"
...
const loadSessions = async () => {
  try {
    ...
    sessions.value = data || [];
    if (!chatStore.currentSession && sessions.value.length) {
      chatStore.openChat(sessions.value[0]);
...
const sendMessage = async () => {
  ...
  payload.append('session_id', chatStore.currentSession.id);
  payload.append('sender_id', userId.value);
  payload.append('receiver_id', chatStore.currentSession.seller.id);
  ...
  try {
    ...
    chatStore.messages = chatStore.messages.filter(msg => msg.id !== tempId);
    if (data && data.id) {
      chatStore.sendMessage(data);
    } else {
      lastLoadedSessionId = null;
      await loadMessages(chatStore.currentSession.id);
    }
    await nextTick();
    if (chatBox.value) chatBox.value.scrollTop = chatBox.value.scrollHeight;
  } catch (err) {
