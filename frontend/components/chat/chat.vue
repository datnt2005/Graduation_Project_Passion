<template>
  ...
  <transition name="fade">
    ...
    >
      <!-- Header -->
      <div
        class="flex items-center justify-between px-4 py-2 bg-[#1BA0E2] text-white"
      >
        <h2 class="font-semibold text-base">
          {{
            selectedSession
              ? `Đang chat với: ${
                  selectedSession?.seller?.store_name || "Cửa hàng"
                }`
              : "Chat với cửa hàng"
          }}
        </h2>
      ...
      <div class="flex-1 flex overflow-hidden" @click="closeContext">
        ...
        <aside class="w-1/3 border-r p-2 bg-gray-50 hidden sm:block">
          ...
          <ul class="space-y-2 overflow-y-auto max-h-[400px] pr-1">
            <li
              ...
              :class="{
                'bg-blue-100': selectedSession?.id === session.id,
                'hover:bg-gray-100': selectedSession?.id !== session.id,
              }"
            >
              <img
        ...
        <section class="flex-1 flex flex-col bg-gray-100 overflow-hidden">
          <!-- Thanh tải thêm khi cuộn lên -->
          <div
            v-if="isLoadingMore"
            class="text-center py-2 text-gray-500 bg-gray-200"
          >
            <i class="fas fa-spinner animate-spin mr-2"></i> Đang tải tin
            nhắn...
          </div>
          ...
          >
            ...
            >
              <!-- Hiển thị khi đang tải tin nhắn cũ -->
              <div
                v-if="isLoadingMore"
                class="text-center py-2 text-gray-500 bg-gray-200 text-sm"
              >
                <i class="fas fa-spinner animate-spin mr-1"></i> Đang tải tin
                nhắn...
              </div>
              ...
              >
                ...
                <p
                  ...
                  :class="{
                    'italic text-gray-300': msg.message_type === 'revoked',
                  }"
                >
                  {{ msg.message }}
                  <span
                ...
                <div
                  v-if="
                    msg.message_type === 'product' && msg.attachments?.length
                  "
                ...
                >
                  ...
                  <div class="flex-1">
                    <div class="text-sm font-semibold line-clamp-2">
                      {{ parseProductName(msg.message) }}
                    </div>
                    <div
                      class="text-xs text-gray-400 line-through"
                      v-if="parseOriginalPrice(msg.message)"
                    >
                ...
                <div
                  v-if="
                    msg.message_type !== 'revoked' && msg.attachments?.length
                  "
                ...
                <div
                  v-if="msg.pending"
                  class="text-xs text-gray-200 italic mt-1"
                >
              ...
              <button
                v-if="
                  msg.sender_type === 'user' && msg.message_type !== 'revoked'
                "
            ...
            <div
              v-if="hasNewMessage"
              class="text-center py-2 bg-blue-100 text-blue-800 cursor-pointer"
              @click="scrollToBottom"
            >
              Có tin nhắn mới! Nhấn để cuộn xuống
            </div>
          </div>
          ...
          <form
            ...
            class="p-3 border-t bg-white flex flex-col gap-3"
          >
            <!-- Preview ảnh -->
            ...
            <div class="flex items-center gap-2">
              ...
              <input
                ...
                class="hidden"
              />
              <button type="button" @click="fileInput.click()" class="text-xl">
                📎
              </button>
              <button
                type="submit"
                class="bg-[#1BA0E2] text-white px-4 py-2 rounded text-sm hover:bg-[#178fca]"
              >
                Gửi
              </button>
            </div>
...
const page = ref(1);
const limit = 20; // Giới hạn số tin nhắn mỗi lần tải

const config = useRuntimeConfig();
const API = config.public.apiBaseUrl;
const DEFAULT_AVATAR =
  "https://pub-3fc809b4396849cba1c342a5b9f50be9.r2.dev/avatars/default.jpg";
const DEFAULT_IMAGE =
  "https://pub-3fc809b4396849cba1c342a5b9f50be9.r2.dev/products/images/default.jpg";
...
const getProductImageUrl = (attachments) => {
  const productAttachment = attachments.find(
    (attachment) =>
  ...
  if (productAttachment) {
    try {
      ...
      if (typeof productAttachment.message_data === "string") {
        const cleanedMessageData = productAttachment.message_data.replace(
          /\\(?![\/\\])/g,
          ""
        );
      ...
      console.warn(
        "⚠️ Lỗi parse message_data:",
        productAttachment.message_data,
        e
      );
...
const parseProductName = (message) => {
  const match = message.match(/^Mình quan tâm sản phẩm: (.*?)(?=\s*-)/) || [
    message,
  ];
...
const parseOriginalPrice = (message) => {
  const match = message.match(/\d+(?:\.\d{2})?(?=\s*-)/);
  return match ? parseFloat(match[0]) : null;
};

// Phân tích giá hiện tại từ message
const parsePrice = (message) => {
  const match = message.match(/\d+(?:\.\d{2})?(?=\s*đ$)/);
...
const loadMessages = async (sessionId, append = false) => {
  if (!token.value || !sessionId || isLoadingMore.value) return;
  ...
  try {
    ...
    console.log("Loaded messages:", data); // Log để debug

    if (data.length) {
      const newMessages = data
        .filter((msg) => !messages.value.some((m) => m.id === msg.id))
        .map((msg) => {
          let attachments = [];
          try {
            ...
            if (msg.message_type === "product" && msg.meta_data) {
              const meta =
                typeof msg.meta_data === "string"
                  ? JSON.parse(msg.meta_data)
                  : msg.meta_data;
              if (meta.file_url) {
                attachments.push({
                  file_type: "image",
            ...
            console.warn(
              "⚠️ Không parse được attachments:",
              msg.attachments,
              e
            );
          }
      ...
      if (append && newMessages.length) {
        messages.value.unshift(...newMessages); // Thêm tin nhắn cũ vào đầu
        page.value++;
        if (newMessages.length < limit) hasMore.value = false;
      } else if (newMessages.length) {
        messages.value.push(...newMessages); // Thêm tin nhắn mới vào cuối
        lastMessageTimestamp.value = data[data.length - 1].created_at;
        hasNewMessage.value = true;
      }

      await nextTick(() => {
        if (chatBox.value && !append) {
...
const handleScroll = () => {
  if (
    ...
    hasMore.value &&
    chatBox.value &&
    chatBox.value.scrollTop < 50
  ) {
    loadMessages(selectedSession.value.id, true);
  }
};
  ...
  async () => {
    ...
    if (chatBox.value) {
      ...
      if (isAtBottom) {
        ...
        hasNewMessage.value = true; // Đánh dấu có tin nhắn mới
      }
    }
  },
  { deep: true }
  ...
  if (newContent && newContent.trim()) {
    try {
      const res = await axios.put(
        `${API}/chat/messages/${msg.id}/action`,
        {
          action: "edit",
          message: newContent,
        },
