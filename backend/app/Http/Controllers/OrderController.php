    ...
    public function syncGhnStatus(Request $request, $orderId)
{
    try {
            ...
            try {
                ...
                if ($retryCount > $maxRetries) {
                    GhnSyncLog::create([
                        ...
                        'ghn_status' => 'none',
                        'success' => false,
                        'message' => "Lỗi từ GHN sau {$maxRetries} lần thử: {$e->getMessage()}"
                    ]);
                    throw $e;
                }
                Log::info("Thử lại GHN API lần {$retryCount}", [
                    'order_id' => $orderId,
                    'error' => $e->getMessage(),
                ]);
                sleep(1);
        ...
        $serviceId = $ghnData['data']['service_id'] ?? null;
        $serviceFee = $ghnData['data']['service_fee'] ?? null;
        ...
        $orderStatus = match ($ghnStatus) {
            'ready_to_pick', 'picking', 'picked', 'delivering' => 'shipping',
            'delivered' => 'delivered',
        ...
        // Đồng bộ shipping_methods nếu có service_id và service_fee
        $shippingMethodId = $serviceId ?? ($order->shipping?->shipping_method_id ?? null);
        if ($serviceId && $serviceFee) {
            ShippingMethod::updateOrCreate(
                ['id' => $serviceId],
                [
                    'name' => $serviceId == 53321 ? 'GHN Tiêu chuẩn' : ($serviceId == 53322 ? 'GHN Nhanh' : 'GHN Dịch vụ #' . $serviceId),
                    'carrier' => 'GHN',
                    'estimated_days' => $serviceId == 53321 ? 3 : ($serviceId == 53322 ? 2 : 3),
                    'cost' => $serviceFee / 100,
                    'status' => 'active',
                ]
            );
            Log::info("Đồng bộ shipping_methods với service_id: {$serviceId}, service_fee: {$serviceFee}");
        ...
        $order->shipping()->updateOrCreate(
            ...
            [
                'tracking_code' => $request->tracking_code,
                'status' => $ghnStatus,
                'shipping_fee' => $serviceFee ? ($serviceFee / 100) : ($order->shipping?->shipping_fee ?? 0),
                'note' => $ghnData['data']['note'] ?? $order->shipping?->note ?? '',
                'shipping_method_id' => $shippingMethodId,
                'estimated_delivery' => $order->shipping?->estimated_delivery ?? now()->addDays($defaultShippingMethod->estimated_days ?? 3)
            ]
        );
    ...
    public function getShippingMethodById(Request $request, $id)
{
    try {
        ...
        if (!$shippingMethod) {
            Log::warning("Shipping method not found for id: {$id}");
            return response()->json([
                'success' => false,
                'message' => "Không tìm thấy phương thức giao hàng với id: {$id}",
            ], 404);
        }
        ...
        Log::error("Lỗi khi lấy shipping method id: {$id}: {$e->getMessage()}", [
            'trace' => $e->getTraceAsString(),
        ]);
        return response()->json([
            'success' => false,
            'message' => 'Lỗi khi lấy thông tin phương thức giao hàng: ' . $e->getMessage(),
        ], 500);
    }
}
